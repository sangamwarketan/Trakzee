...
public class DatePicker {
	...
	public static void DatePicker_GenericMethod_WithoutDropDown(
			...
			WebElement path_YearDateToggleBtn, WebElement selectMonth, WebElement iconNextMonth,
			WebElement iconPreviousMonth, List<WebElement> path_dateGrid, List<WebElement> path_yearGrid,
		...
		while (true) {
			...
			if (currentmonthyear.equals(usergiven_month_year)) {
				log = ("Displayed month year: " + currentmonthyear + " User month year: " + usergiven_month_year
						+ " Matched");
				logger.info(log);
				Thread.sleep(1000);
				break;
			...
			if (displayedMonth == inputMonth) {
				...
				while (displayedMonth > inputMonth) {
					try {
						...
						if (callerMethodName.equals("setPaymentStartDate")
								|| callerMethodName.equals("setPaymentEndDate")) {
