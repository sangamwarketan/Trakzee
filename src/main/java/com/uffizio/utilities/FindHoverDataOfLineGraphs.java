...
public class FindHoverDataOfLineGraphs {
		...
		if (xAxisUserValue == null || xAxisUserValue == "") {
			...
			for (WebElement ydata : listGraphData_WRT_xaxis) {
				...
				try {
					...
					Thread.sleep(300);
					tooltipText(driver, toolTip_TitleAddress);
	...
	//TO FIND TOOL-TIP TITLE
		public static void tooltipText(WebDriver driver, String toolTip_TitleAddress) throws InterruptedException {
			try {
				Thread.sleep(500);
				WebElement tooltips_Title = driver.findElement(By.xpath(toolTip_TitleAddress));
				Thread.sleep(500);
				String tooltips_TitleValues = tooltips_Title.getText();
		...
		public static void tooltipTitle(WebDriver driver, String toolTip_TitleAddress) throws InterruptedException {
			try {
				...
				if(tooltip_TitleValues.equals("") || tooltip_TitleValues.equals(null)) {
					tooltip_TitleValues = tooltips_Title.getAttribute("title");
				}
				reportLogger.info("Mouse move on the element: " + tooltip_TitleValues);
				//logger.info("Mouse move on the element: " + tooltip_TitleValues);
				Thread.sleep(500);
			} catch (Exception e) {
				reportLogger.info("Exception from tooltipTitle: " + e.getMessage());
				//logger.info("Exception from tooltipTitle: " + e.getMessage());
			}
		}
